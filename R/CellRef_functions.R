# R functions to implement the guided single cell reference (CellRef) construction
...
doDataIntegration_mnn <- function(object, integration.batch, ident.var=NULL, 
  ...
  if (length(vars.to.regress)>0) {
    ...
    cds <- preprocess_cds(cds, method="PCA", num_dim = npcs, residual_model_formula_str=formula_str)
  } else {
